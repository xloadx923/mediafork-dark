...
function displayService(){
    ...
    services.forEach(function (service){
        service.addEventListener('click', function(event){
            ...
            modal.addEventListener('click', function(){ contModal.remove();bodyToggle(); });
        });
    });
...
mainNav.addEventListener('click', function(event) {  if(event.target.getAttribute('href')) toggleNav(); });

const portfolioImgs = document.querySelectorAll('#portfolio-grid li a');
portfolioImgs.forEach( function(portfolioImg){ portfolioImg.addEventListener('click', event => event.preventDefault()); });
